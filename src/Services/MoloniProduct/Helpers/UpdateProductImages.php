...
class UpdateProductImages
{
    ...
    private function handle(): void
    {
        ...
        $variables = [
            'companyId' => Context::$MOLONI_ON_COMPANY_ID,
            'data' => [
    ...
    private function saveImagesMetaTag($updatedProduct)
    {
        if (!empty($updatedProduct['img']) && !empty($this->images[0]['id'])) {
            update_post_meta((int)$this->images[0]['id'], '_molonion_file_name', $updatedProduct['img']);
        }
        ...
        foreach ($updatedProduct['variants'] as $variant) {
            ...
            update_post_meta($imageId, '_molonion_file_name', $variant['img']);
        }
    }
